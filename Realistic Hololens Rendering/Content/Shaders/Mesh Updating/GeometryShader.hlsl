...
void main(triangle GeometryShaderInput inputs[3], uint primitiveID : SV_PrimitiveID, inout TriangleStream<GeometryShaderOutput> outputStream)
{
	...
	for (int i = 0; i < 3; i++)
	{
		uvs[i] = GetInnerUV(i, primitiveID, OldOffset, OldSize);
		innerVertices[i].Position = float4(UVToPosition(GetInnerUV(i, primitiveID, NewOffset, NewSize)), 0.0, 1.0);
		innerVertices[i].UV = uvs[i];
		outerVertices[i].Position = float4(UVToPosition(GetOuterUV(i, primitiveID, NewOffset, NewSize)), 0.0, 1.0);
		outerVertices[i].UV = uvs[i];
	}
	[unroll]
	for (int i = 0; i < 3; i++)
	{
